"""XHS Mock Provider - connects to official-sim for order/shipment/after-sale data."""
import httpx
import os
...
from providers.xhs.mock.platform_sim_adapter import (
    adapt_platform_sim_order,
    adapt_platform_sim_shipment,
    adapt_platform_sim_refund,
    adapt_platform_sim_inventory,
)
from providers.xhs.mock.mapper import map_order, map_shipment, map_after_sale, map_inventory
...
class XhsMockProvider(OrderProvider, ShipmentProvider, AfterSaleProvider):
    ...
    def get_order(self, order_id: str) -> OrderDTO:
        ...
        payload = _handle_response(response, "order", order_id, "xhs")
        order_data = payload.get("data", {}).get("order", {})
        adapted = adapt_platform_sim_order(order_data)
        return map_order(adapted, "xhs")
