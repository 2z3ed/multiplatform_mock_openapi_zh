"""Adapter layer: platform-sim Query API format → mock-platform-server format for XHS."""
...
def adapt_platform_sim_order(order_data: dict) -> dict:
    ...
    xhs_status = status_code_map.get(raw_status, raw_status)

    return {
        "orderId": order_data.get("order_id", ""),
        "orderStatus": xhs_status,
        ...
        "freightAmount": float(order_data.get("postage", 0)),
        "discountAmount": 0.0,
