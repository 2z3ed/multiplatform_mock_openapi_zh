"""Adapter layer: platform-sim Query API format → mock-platform-server format for WeCom KF."""
...
def adapt_platform_sim_order(order_data: dict) -> dict:
    ...
    items = []
    for item in order_data.get("items", order_data.get("products", [])):
        items.append({
            "skuId": item.get("product_id", item.get("item_id", item.get("sku_id", ""))),
    ...
    status_code_map = {
        ...
        "confirmed": "WAIT_SELLER_SEND",
        "fulfilled": "WAIT_BUYER_CONFIRM",
        "completed": "FINISHED",
        "cancelled": "CANCELLED",
    }
    raw_status = order_data.get("status", "")
    wc_status = status_code_map.get(raw_status, raw_status)

    return {
        "orderId": order_data.get("order_id", ""),
        "orderStatus": wc_status,
...
def adapt_platform_sim_inventory(inventory_data: list, order_id: str) -> dict:
    ...
    for item in inventory_data:
        items.append({
            ...
            "stockState": item.get("stock_state", ""),
            "quantity": item.get("quantity", 0),
            "warehouseName": item.get("warehouse_name", ""),
        })
